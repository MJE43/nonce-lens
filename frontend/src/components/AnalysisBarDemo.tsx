...
export const AnalysisBarDemo: React.FC = () => {
  ...
  const [currentRange, setCurrentRange] = useState<[number, number]>([60000, 70000]);
  const [minMultiplier, setMinMultiplier] = useState(1);
  const [pinnedBuckets, setPinnedBuckets] = useState<number[]>([11200, 48800, 400]);
  ...
  const currentStats = focusedBucket ? mockStatsByPinnedBuckets[focusedBucket.toString()] || null : null;
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="p-8 space-y-4">
        ...
        <ul className="list-disc list-inside space-y-1 text-sm text-muted-foreground">
          ...
          <li>Navigate between nonce ranges using keyboard shortcuts (J/K) or buttons</li>
        ...
        <div className="mt-8 p-4 border rounded-lg bg-muted/50">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              <strong>Focused Bucket:</strong> {focusedBucket ? `${focusedBucket}x` : 'None'}
            </div>
            <div>
              <strong>Range:</strong> {currentRange[0].toLocaleString()}–{currentRange[1].toLocaleString()}
            </div>
            <div>
              <strong>Min Multiplier:</strong> {minMultiplier}x
            </div>
            <div>
              <strong>Pinned Buckets:</strong> {pinnedBuckets.length > 0 ? pinnedBuckets.map(b => `${b}x`).join(', ') : 'None'}
            </div>
            <div>
